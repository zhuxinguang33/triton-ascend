...
# Test purpose: Verify MLIR generation of 4-layer nesting, inner C depends on outer V (V2C cross-layer) under float16
# ----------------------------------------------------------------------------
@triton.jit
def sdf22(
    a_ptr,
...
def _build_sdf22_signature(dtype_str):
    """Build the argument type signature for the SDF22 kernel."""
    return {
        "a_ptr": f"*{dtype_str}",
        "b_ptr": f"*{dtype_str}",
        "c_ptr": f"*{dtype_str}",
...
def test_sdf22():
    """SDF22: Verify float16 kernel compilation generates correct MLIR code.

    Test steps:
      1. Compile sdf22 kernel to MLIR
