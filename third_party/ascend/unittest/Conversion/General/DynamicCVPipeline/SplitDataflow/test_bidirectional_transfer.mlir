...
// CHECK: hivm.hir.fixpipe {dma_mode = #hivm.dma_mode<nz2nd>, ssbuffer.block_id = 1 : i32, ssbuffer.core_type = "CUBE", ssbuffer.crossCoreDeps = [1 : i32, 1 : i32], ssbuffer.transfer_id = 1 : i32} ins(%[[MATMUL_3]] : tensor<256x256xf32>) outs(%[[ALLOC_0]] : memref<256x256xf32, #hivm.address_space<ub>>)
...
// CHECK: %[[MEMSPACECAST:[a-z0-9_]+]] = memref.memory_space_cast %[[ALLOC_1]] {ssbuffer.block_id = 2 : i32, ssbuffer.core_type = "VECTOR", ssbuffer.crossCoreDeps = [1 : i32, 0 : i32], ssbuffer.transfer_id = 1 : i32} : memref<256x256xf32, #hivm.address_space<ub>> to memref<256x256xf32>
...
// CHECK: hivm.hir.copy ins(%[[RESHAPE_4]] : tensor<32x16x16x8xf32>) outs(%[[ALLOC_5]] : memref<32x16x16x8xf32, #hivm.address_space<cbuf>>) {ssbuffer.block_id = 2 : i32, ssbuffer.core_type = "VECTOR", ssbuffer.crossCoreDeps = [0 : i32, 1 : i32], ssbuffer.transfer_id = 0 : i32}
...
// CHECK: %[[MEM_7:[a-z0-9_]+]] = hivm.hir.convert_layout %[[ALLOC_6]] output_shape [256, 256] {dstLayout = #hivm.data_layout<ND>, srcLayout = #hivm.data_layout<nZ>, ssbuffer.block_id = 3 : i32, ssbuffer.core_type = "CUBE", ssbuffer.crossCoreDeps = [0 : i32, 0 : i32], ssbuffer.transfer_id = 0 : i32} : (memref<32x16x16x8xf32, #hivm.address_space<cbuf>>) -> memref<256x256xf32, #hivm.address_space<cbuf>>
